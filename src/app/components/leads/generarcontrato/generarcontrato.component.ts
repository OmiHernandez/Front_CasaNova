...
import { UploadpdfService } from 'src/app/services/uploadpdf.service';
...
export class GenerarcontratoComponent {
  ...
  constructor(
    ...
    this.leadApiService.getLeadsCotizado().subscribe(
      (data: any[]) => {
        data.sort((a, b) => {
          return (
            new Date(a.currentDate).getTime() -
            new Date(b.currentDate).getTime()
          );
        });
  ...
  async getDatosCompletos() {
    for (let lead of this.ListaLeads) {
      ...
      await this.inventarioApi
        .getInventarioById(lead.idinteres)
        .subscribe((data) => {
  ...
  async createPdf(
    ...
    await this.cotizacionesService
      .getCotizacionPorIdUsuario(id)
      .subscribe(async (data2) => {
        console.log(data2._id, 'cotizaciones');

        await this.cotizacionesService
          .getMensualidadesPorId(data2._id)
          .subscribe(async (data3) => {
            ...
            if (data2.tieneEnganche == 'Si') {
              ...
              rowData.push('0');
              rowData.push(
                data2.enganche.toLocaleString('es-MX', {
                  style: 'currency',
                  currency: 'MXN',
            ...
            data3.forEach((row) => {
              ...
              var textonum = this.numeroATexto(
                parseFloat(row.pago.replace(',', ''))
              );

              rowData.push(textonum + ' 00/100');
              rowData.push(
                row.es_enganche_o_mensualidad.charAt(0).toUpperCase() +
                  row.es_enganche_o_mensualidad.slice(1)
              );
            ...
            await this.inventarioApi
              ...
              .subscribe((data) => {
                ...
                costototal =
                  leadparam.preciovente *
                  leadparam.medidas *
                  (1 - leadparam.descuento / 100);
                ...
                if (medidas > 1) {
                  textmetros =
                    ...
                    ' de ' +
                    medidas +
                    ' metros de construcción que se construirá en el lote ';
                } else {
                  textmetros =
                    modelo +
                    ' de ' +
                  ...
                  content: [
                    {
                      text: [
                        {
                          text:
                            'CONTRATO DE PROMESA QUE CELEBRAN POR UNA PARTE LA EMPRESA DENOMINADA “_______” REPRESENTADA EN ESTE ACTO POR EL SEÑOR _______ A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ “EL VENDEDOR” Y POR LA OTRA ' +
                            nombre +
                            ', A QUIEN EN ADELANTE SE LE DENOMINARÁ COMO “EL CLIENTE” AL TENOR DE LAS SIGUIENTES DECLARACIONES Y CLAUSULAS:\n\n',
                          bold: true,
                        },
                      ],
                    },
                    ...
                    },
                    {
                      text: [
                        { text: 'A) .-', bold: true },
                        'Que  es una sociedad mercantil debidamente constituida mediante Escritura Pública número _______, de fecha _______, pasada ante la fe del _______, Notario Público Número _______ del Estado de _______, y que su representante _______, con carácter de _______, cuenta con las facultades necesarias para la celebración del presente contrato, mismas que se desprenden de la  escritura pública descrita en este inciso, en la cual, le fueron otorgados poderes amplísimos de representación de la sociedad, declarando bajo protesta de decir verdad que las facultades que le fueron conferidas no le han sido modificadas o revocadas en forma alguna. \n\n',
                      ],
                    },
                    {
                      text: [
                        { text: 'B) .-', bold: true },
                        'Que tiene su domicilio en _______, mismo que señala para todos los efectos legales relativos a la celebración del presente Contrato.  \n\n',
                      ],
                    },
                    {
                      text: [
                        ...
                        'ubicado en la etapa ',
                        {
                          text:
                            ...
                            ' del Fraccionamiento ' +
                            frac +
                            ', con las siguiente superficie de terreno, medidas y colindancias; superficie de ' +
                            medTer +
                            ', colindando ' +
                            colind +
                            ' y para efectos del presente contrato, se le denominará como "EL INMUEBLE".\n\n',
                          bold: true,
                        },
                      ],
                    },
...

                    {
                      text: [
                        ...
                        'Ser ' +
                          ...
                          domicilio +
                          ' y que tiene capacidad legal para obligarse y celebrar el presente contrato de promesa de compraventa.\n\n',
                      ],
                    },
...

                    {
                      text: [
                        {
                          text: 'SEGUNDA: TERMINO DEL CONTRATO .- "LAS PARTES"',
                          bold: true,
                        },
...

                    {
                      text: [
                        {
                          text: 'CUARTA: FORMALIZACIÓN DEL CONTRATO DEFINITIVO.-',
                          bold: true,
                        },
                    ...
                    },
                    {
                      text: [
                        { text: 'Banco:\n\n', bold: true },
                        { text: 'Titular: _______\n\n', bold: true },
                        { text: 'Cuenta Bancaria: _______\n\n', bold: true },
                        { text: 'Cuenta CLABE: _______\n\n', bold: true },
                        {
                          text: 'Titular de la cuenta : _______\n\n',
                          bold: true,
                        },
                        {
                    ...
                    },
                    {
                      text: [
                        {
                          text: 'OCTAVA: ENTREGA DE "EL INMUEBLE".- ',
                          bold: true,
                        },
                    ...
                    },
                    {
                      text: [
                        {
                          text: 'NOVENA: GASTOS DE ESCRITURACIÓN E IMPUESTOS.- ',
                          bold: true,
                        },
                    ...
                    },
                    {
                      text: [
                        {
                          text: 'DECIMA SEGUNDA: LEGISLACIÓN APLICABLE.- ',
                          bold: true,
                        },
                        'La Procuraduría Federal del Consumidor (PROFECO) es competente en la vía administrativa para resolver cualquier controversia que se suscite sobre la interpretación o cumplimiento del presente contrato.  Sin perjuicio de lo anterior, las partes acuerdan que para la interpretación y cumplimiento de lo pactado en el presente contrato, las partes se someten expresamente a la jurisdicción y competencia de los tribunales de _______, renunciando expresamente a cualquier otro fuero que en razón de sus domicilios presentes o futuros o por cualquier otra causa, pudiera corresponderles.\n\n',
                      ],
                    },
                    {
                      text: [
                        ...
                        'de acuerdo con su contenido, lo firman por duplicado, ante los testigos que también firman, en _______ a ' +
                          fecha +
                          '.\n\n\n\n',
                      ],
                    },
                ...
                try {
                  ...
                  pdf.download('Contrato_generado_' + nombre + '.pdf');
                  pdf.getBlob((blob: Blob) => {
                    this.uploadpdfService.uploadContrato(blob, id, 'Adjunto encontrará el contrato generado en formato PDF.', 'Contrato_generado_' + nombre + '.pdf').subscribe(
                      (response) => {
                        Swal.fire({
                          title: 'Contrato generado y enviado',
                          text: 'Contrato generado y enviado correctamente al cliente.',
                          icon: 'success',
                        });
                      },
                      (error) => {
                        Swal.fire({
                          icon: 'error',
                          title: 'Error al generar o enviar el contrato',
                          text: 'Ocurrió un error al generar o enviar el contrato. Por favor, inténtalo de nuevo.',
                        });
                      }
                    );
                  });
  ...
  avanzarlead(id: any) {
    this.leadApiService.anvanzarLead(id).subscribe(
      (data: any[]) => {
  ...
  async regresarLead(id: any, idinv: any): Promise<void> {
    await this.cotizacionesService
      .borrarCotizacionPorIdUsuario(id)
      .subscribe(async (data2) => {
        console.log(data2._id, 'cotizaciones');

        await this.cotizacionesService
          .borrarPorIdCotizacion(data2._id)
          .subscribe(async (data3) => {});
      });
    ...
    this.leadApiService.regresarLead(id).subscribe(
      (data: any[]) => {
        this.inventarioApi
          .putApartarInventario(est, idinv)
          .subscribe((data2) => {
            const Toast = Swal.mixin({
              toast: true,
              position: 'center',
