...
export class FirmadoNotarialComponent {
  ...
  constructor(
    ...
    this.leadApiService.getLeadsFirmado().subscribe(
      (data: any[]) => {
  ...
  async createPdf2(
    ...
    var docDefinition = {
      content: [
        ...
        },
        {
          text: [
            {
              text: 'REFERENCIA: Fideicomiso No. _______',
              alignment: 'center',
            },
          ],
        },
        ...
        {
          text: ['NOTARIO: _______'],
        },
        {
          text: ['TELEFONO: _______'],
        },
        {
          text: ['RESPONSABLE: _______     CORREO: _______\n\n'],
        },
        {
          text: [
        ...
        {
          canvas: [
            { type: 'line', x1: 0, y1: 0, x2: 500, y2: 0, lineWidth: 2 },
          ],
        },
        {
          text: [
            {
              text: '\n\nFIDEICOMITENTE Y FIDEICOMISARIO ' + fide,
              alignment: 'center',
            },
          ],
        },
        {
          text: [{ text: sociedad, alignment: 'center' }],
        },
        {
          text: [{ text: nombre, alignment: 'center' }],
        },
      ],
    };
    ...
    try {
      ...
      pdf.download('InstruccionNotarial_'+ adquiere + '_' + this.obtenerFechaActualPDF() + '.pdf');
      pdf.getBlob((blob: Blob) => {
        this.uploadpdfService.uploadContrato(blob, id, 'Adjunto se encuentra la generación de la instrucción notarial para el cliente ' + adquiere + '\nGenerado el dia: ' + this.obtenerFechaActual(), 'InstruccionNotarial_'+ adquiere + '_' + this.obtenerFechaActualPDF() + '.pdf').subscribe(
          (response) => {
            Swal.fire({
              title: 'Instrucción notarial generado y enviado',
              text: 'Instrucción notarial generado y enviado correctamente al cliente.',
              icon: 'success',
            });
          },
          (error) => {
            Swal.fire({
              icon: 'error',
              title: 'Error al generar o enviar la instrucción notarial',
              text: 'Ocurrió un error al generar o enviar la instrucción notarial. Por favor, inténtalo de nuevo.',
            });
          }
        );
      });
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error al generar la instrucción notarial',
        text: 'Ocurrió un error al generar la instrucción notarial. Por favor, inténtalo de nuevo.',
      });
    }
  }
  ...
  async createPdf(
    ...
    var docDefinition = {
      content: [
        ...
        {
          text: [{ text: this.suscribef, alignment: 'center' }],
        },
      ],
    };
    ...
    try {
      ...
      pdf.download('InstruccionNotarial_'+ adquiere + '_' + this.obtenerFechaActualPDF() + '.pdf');
      pdf.getBlob((blob: Blob) => {
        this.uploadpdfService.uploadContrato(blob, id, 'Adjunto se encuentra la generación de la instrucción notarial para el cliente ' + adquiere + '\nGenerado el dia: ' + this.obtenerFechaActual(), 'InstrucciónNotarial_'+ adquiere + '_' + this.obtenerFechaActualPDF() + '.pdf').subscribe(
          (response) => {
            Swal.fire({
              title: 'Instrucción notarial generado y enviado',
              text: 'Instrucción notarial generado y enviado correctamente al cliente.',
